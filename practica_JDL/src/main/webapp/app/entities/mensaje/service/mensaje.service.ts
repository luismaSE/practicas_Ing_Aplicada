...
type RestOf<T extends IMensaje | NewMensaje> = Omit<T, 'fechaPublicacion'> & {
...
export class MensajeService {
  ...
  protected convertDateFromClient<T extends IMensaje | NewMensaje | PartialUpdateMensaje>(mensaje: T): RestOf<T> {
    return {
      ...mensaje,
      fechaPublicacion: mensaje.fechaPublicacion?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMensaje: RestMensaje): IMensaje {
    return {
      ...restMensaje,
      fechaPublicacion: restMensaje.fechaPublicacion ? dayjs(restMensaje.fechaPublicacion) : undefined,
    };
  }
