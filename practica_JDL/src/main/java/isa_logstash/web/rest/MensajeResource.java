...
/**
 * REST controller for managing {@link isa_logstash.domain.Mensaje}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Mensaje> result = mensajeRepository
            ...
            .map(existingMensaje -> {
                if (mensaje.getAutor() != null) {
                    existingMensaje.setAutor(mensaje.getAutor());
                }
                if (mensaje.getFechaPublicacion() != null) {
                    existingMensaje.setFechaPublicacion(mensaje.getFechaPublicacion());
                }
                if (mensaje.getTextoMensaje() != null) {
                    existingMensaje.setTextoMensaje(mensaje.getTextoMensaje());
                }
                if (mensaje.getEtiquetas() != null) {
                    existingMensaje.setEtiquetas(mensaje.getEtiquetas());
                }
